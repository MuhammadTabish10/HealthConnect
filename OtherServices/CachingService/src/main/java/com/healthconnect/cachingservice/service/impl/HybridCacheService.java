...
public class HybridCacheService implements CacheService {
    ...
    public HybridCacheService(@Qualifier("caffeineCacheManager") CacheManager caffeineCacheManager,
    ...
    public <T> T getFromCache(String cacheName, String key, Class<T> type) {
        ...
        if (caffeineCache != null) {
            ...
            if (cachedValue != null) {
                ...
                return type.cast(cachedValue.get());
            }
        }
        ...
        if (redisCache != null) {
            ...
            if (redisValue != null) {
                T result = type.cast(redisValue.get());
                if (caffeineCache != null) {
                    caffeineCache.put(key, result);
                    log.debug(LoggingConstants.CACHE_PUT_SUCCESS, key, cacheName);
                }
                log.debug(LoggingConstants.CACHE_HIT, key, cacheName);
                return result;
            }
